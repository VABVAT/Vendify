...
export default function InpOup({sendMode, prod, index }) {
    ...
    return (<>
        {prod.status === "PENDING" ? <div key={index} className="relative flex w-[70%] h-[200px] items-center border border-gray-800 rounded-lg shadow-md mb-4 bg-Gld overflow-hidden">
  ...
  <div className="h-[100%] absolute  w-full z-50 text-center text-4xl font-bold py-2 bg-opacity-75 bg-black text-white flex justify-center items-center">
    {prod.status === "ACCEPTED" ? (
        <div>
                ...
                <div className="text-3xl text-green-300"> {jwtDecode(localStorage.getItem('token')).email === prod.originalSeller ? <span> Contact: {prod.offeredBy === prod.originalSeller ? prod.offeredTo : prod.offeredBy}</span> : <span> {prod.offeredBy === prod.originalSeller ? prod.offeredBy : prod.offeredTo} </span>}</div>
...
{N === true ? <><div className="fixed inset-0 w-screen h-screen bg-black flex justify-center items-center z-50 bg-opacity-50"> 
